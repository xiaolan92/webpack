...
rewritePattern('\\p{Unified_Ideograph}', 'u', {
    ...
    /**
 *
 * @method {Object} entries 多文件单入口
 *
 */
    entries = function (){
        let jsDir = path.resolve(__dirname,"../src"),
            entriesFiles = glob.sync(jsDir + "/*.js"),
            map = {};
        for(let i = 0;i < entriesFiles.length;i++){
            let filePath = entriesFiles[i],
...
Object.keys(entries()).forEach(function (name){
    let plugin = new Htmlwebpackplugin({
        template:"./src/index." + name + ".tpl.html",
        filename:name + ".html",
        inject:"body",
        showErrors:true,
        cache:true,
